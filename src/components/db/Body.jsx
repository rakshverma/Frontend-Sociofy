...
function Body() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/posts/${email}`);
...
const navigateToFriendProfile = (friendEmail) => {
  ...
  window.location.href = `/user-profile/${friendEmail}`;
  ...
  const handleLike = async (postId) => {
    try {
      await axios.post(`http://localhost:5000/like/${postId}`, { email });
      fetchPosts();
    } catch (error) {
  ...
  const handleComment = async (postId) => {
    ...
    try {
      await axios.post(`http://localhost:5000/comment/${postId}`, { email, comment });
  ...
  const handleDelete = async (postId) => {
    try {
      await axios.delete(`http://localhost:5000/delete/${postId}`);
      fetchPosts();
    } catch (error) {
  ...
  const fetchFriends = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/friends/${email}`);
