...
function Nav() {
  ...
  useEffect(() => {
    const fetchFriendRequests = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/friend-requests/${userEmail}`);
  ...
  const handleAcceptFriendRequest = async (requestId) => {
    try {
      ...
      const response = await axios.post(`http://localhost:5000/accept-friend-request/${requestId}`, {
        email: userEmail
      });
  ...
  const handleRejectFriendRequest = async (requestId) => {
    try {
      await axios.post(`http://localhost:5000/reject-friend-request/${requestId}`, {
        email: userEmail
      });
    ...
    <nav className="bg-white shadow-md fixed top-0 left-0 right-0 z-50 h-14">
      <div className="max-w-5xl mx-auto px-2">
        <div className="flex items-center justify-between h-full">
          {/* Logo/Brand */}
          <div className="flex-shrink-0 flex items-center py-3 pl-0">
