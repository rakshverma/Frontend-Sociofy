...
function UserProfile() {
  ...
  useEffect(() => {
    const fetchUserAndPosts = async () => {
      try {
        ...
        // Fetch user's posts
        const postsResponse = await axios.get(`http://localhost:5000/posts/${email}`);
        setPosts(postsResponse.data);

        // Check friend status if viewing someone else's profile
        if (currentUserEmail && currentUserEmail !== email) {
          const statusResponse = await axios.get(
            `http://localhost:5000/friend-status?userEmail=${currentUserEmail}&friendEmail=${email}`
          );
  ...
  const handleLike = async (postId, isLiked) => {
    try {
      if (isLiked) {
        await axios.post(`http://localhost:5000/unlike/${postId}`, {
          email: currentUserEmail,
        });
      } else {
  ...
  const handleComment = async (postId) => {
    try {
      ...
      await axios.post(`http://localhost:5000/comment/${postId}`, {
        email: currentUserEmail,
